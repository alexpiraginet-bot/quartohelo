...
export async function getSiteContent(): Promise<SiteContent> {
  ...
  try {
    const { data } = await supabase.from("site_content").select("data").eq("id", "landing").maybeSingle();
...
export async function getGuide(): Promise<GuideMeta> {
  ...
  try {
    const { data } = await supabase.from("guide_meta").select("data").eq("id", "guia").maybeSingle();
...
export async function getCategories(): Promise<Category[]> {
  ...
  try {
    ...
    const { data: items } = await supabase.from("items").select("*").order("order");
